...
{
	// GBG.VisualPlayable.AnimationLayer.SetMixSpeeds
[UnitCategory("Visual Playable/Animation Layer")]
[UnitTitle("Set Mix Speeds")]
internal class Node_AnimationLayer_SetMixSpeeds : Unit
{
	// Input trigger
...
Input_speeds = ValueInput<IReadOnlyList<Single>>(nameof(Input_speeds));
...
var Value_Input_speeds = flow.GetValue<IReadOnlyList<Single>>(Input_speeds);

Value_Input_AnimationLayer.SetMixSpeeds(Value_Input_speeds);
} catch(Exception e) {
